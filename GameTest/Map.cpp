	...
	for (int z = 0; z < length; z++)
	{
		for (int x = 0; x < width; x++)
		{
			for (int y = 0; y < rand() % 5 + 1; y++)
			{
				...
				//std::cout << "Initializing Block at " << Vector3(x, y, z) << std::endl;
			}
		}
	}
	for (Block block : blocks)
	{
		blockCols.push_back(Block(block.bb.LeftUpBack, 0.2f, Color(0.0f, 0.0f, 1.0f), false));
		blockCols.push_back(Block(block.bb.RightDownFront, 0.2f, Color(0.0f, 1.0f, 0.0f), false));
	}
}

void Map::Render(int rendermode)
{
	if (rendermode == Renderer::renderModes::Collision)
	{
		glBegin(GL_LINES);
